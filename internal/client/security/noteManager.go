...
var globalNoteManager *NoteManager = nil

func NewNoteManager(logger common.Logger) *NoteManager {
	if globalNoteManager == nil {

		globalNoteManager = &NoteManager{logger: logger}
	}
...
func (manager *NoteManager) InitParametrs(login []byte, password []byte) {
	...
	manager.cryptoKey = generateCryptoKey(login, password)
}

func (manager *NoteManager) GetLogin() string {
	return manager.login
}
func (manager *NoteManager) CreateNewNotes(plainText []byte, description string, metadata *clientCommon.Metadata) *common.Note {
	...
	common.SaveDataToStream(dataForEncrypt, &actualIndex, plainText)
